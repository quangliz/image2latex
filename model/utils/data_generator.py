    ...
    def __iter__(self):
        with open(self._filename) as f:
            for line in f:
                ...
                # Handle im2latex format: id hash type
                if len(line) >= 3:
                    # The first element is the ID, second is the hash (filename)
                    path_img, id_formula = line[1] + ".png", line[0]
                else:
...
class DataGenerator(object):
    ...
    def _process_instance(self, example):
        ...
        img = Image.open(self._dir_images + "/" + img_path)
