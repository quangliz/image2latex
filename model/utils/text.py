    ...
    try:
        with open(filename, 'r', encoding='utf-8') as f:
            for idx, line in enumerate(f):
                # Check if file is in im2latex format
                if filename.endswith('im2latex_formulas.lst'):
                    formulas[idx+1] = line.strip()  # im2latex uses 1-indexed IDs
                else:
                    formulas[idx] = line.strip()
    except UnicodeDecodeError:
        # Try with latin-1 encoding which can handle all byte values
        with open(filename, 'r', encoding='latin-1') as f:
            for idx, line in enumerate(f):
                # Check if file is in im2latex format
                if filename.endswith('im2latex_formulas.lst'):
