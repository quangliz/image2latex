...
import sys, os, argparse, logging, subprocess, shutil, io

def is_ascii(str):
    try:
        str.encode('ascii')  # Changed from decode to encode since we're working with strings
        return True
    except UnicodeError:
        return False
...
def main(args):
    ...
    with open(temp_file, 'w', encoding='utf-8') as fout:
        for encoding in encodings:
            try:
                with open(output_file, 'r', encoding=encoding) as fin:
                    for line in fin:
                        fout.write(line.replace('\r', ' ').strip() + '\n')  # delete \r
                break  # If successful, break the loop
            except UnicodeError as e:
                logging.warning(f"Failed to open with encoding {encoding}: {e}")
                if encoding == encodings[-1]:  # If this was the last encoding to try
                    logging.error("Could not decode the file with any of the attempted encodings")
                    raise
