...
def process_args(args):
    ...
    parser.add_argument('--use-grayscale', dest='use_grayscale',
                        action='store_true', default=True,
                        help=('Convert images to grayscale during preprocessing.'
                        ))
    parameters = parser.parse_args(args)
    return parameters

def main_parallel(l):
    filename, postfix, output_filename, crop_blank_default_size, pad_size, buckets, downsample_ratio, use_grayscale = l
    ...
    if use_grayscale:
        status = grey_scale(output_filename, output_filename)
        if not status:
            logging.info('%s failed to convert to grayscale!'%filename)
...
def main(args):
    ...
    use_grayscale = parameters.use_grayscale
    ...
    results = pool.map(main_parallel, [(filename, postfix, os.path.join(output_dir, os.path.basename(filename)), crop_blank_default_size, pad_size, buckets, downsample_ratio, use_grayscale) for filename in filenames])
    pool.close() 
    pool.join() 
