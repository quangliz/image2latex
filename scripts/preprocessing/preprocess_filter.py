        ...
        with open(data_path, 'r') as fdata:
            for line in fdata:
                ...
                if len(line_strip) > 0:
                    ...
                    # Convert line_idx to int for safety
                    try:
                        line_idx_int = int(line_idx)
                    except ValueError:
                        logging.warning(f"Invalid label index in line: {line_strip}")
                        num_discard += 1
                        continue
                    ...
                    if parameters.filter:
                        ...
                        if not os.path.exists(img_path):
                            logging.warning(f'{os.path.basename(img_path)} does not exist!')
                            num_nonexist += 1
                            continue
                        ...
                        if line_idx_int >= len(labels):
                            logging.warning(f'{os.path.basename(img_path)} has invalid label index {line_idx_int} (max {len(labels)-1})')
                            num_discard += 1
                            continue
                    else:
                        w = 0
                    ...
                    if (not parameters.filter) or (w <= parameters.max_width and h <= parameters.max_height):
                        if parameters.filter:
                            label = labels[line_idx_int]
                            if len(label.strip()) == 0:
                                logging.info(f'{os.path.basename(img_path)} discarded due to cannot-be-parsed formula!')
                                num_discard += 1
                                continue
                            if len(label.strip().split()) > parameters.max_tokens:
                                logging.info(f'{os.path.basename(img_path)} discarded due to too many tokens!')
                                num_discard += 1
                                continue
                        fout.write(f'{os.path.basename(img_path)} {line_idx}\n')
                    else:
                        logging.info(f'{os.path.basename(img_path)} discarded due to large image size!')
                        num_discard += 1
    logging.info(f'{num_discard} discarded. {num_nonexist} not found in {image_dir}.')
